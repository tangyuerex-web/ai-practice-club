...
- 用户自己填写的姓名、绰号、介绍和隐藏留言不会被自动翻译；只有尚未修改的示例内容会跟随语言切换。
...
填写的姓名、绰号、介绍、颜色、互动方式、隐藏留言和当前页码都会自动保存为浏览器草稿。前后翻页不会清空内容，意外刷新或关闭后重新打开也会恢复；发布到 Supabase 的跨设备数据逻辑保持不变。

## 生成后的个人主页

- 第一页展示姓名、绰号和个人介绍，并通过“Know more about me”进入下一页。
- 第二页是全屏互动空间：中央显示姓名的首个字母，互动提示位于右侧。
