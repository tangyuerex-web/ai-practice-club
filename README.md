...
填写的姓名、绰号、介绍、颜色、互动方式、补充信息和当前页码会保存在当前标签页的会话草稿中。前后翻页或发布前刷新当前标签页不会清空内容；成功发布、关闭标签页后重新打开 Builder，都会恢复默认信息，避免沿用上一位使用者填写的内容。发布到 Supabase 的跨设备数据逻辑保持不变。
...
- 使用者选择的主题色会直接写入个人主页容器，并同步应用到两页背景强调、字母标识、按钮、滑杆与互动图样。
- 支持底部分页滑杆、分页按钮和手机左右滑动。
- 个人主页不展示社团主站品牌或模板说明，更像使用者自己的独立主页。

## 手机端显示

- Builder 在手机上使用独立的大字号、单栏问题布局，输入框和按钮均保持易于触碰的尺寸。
- 页面使用真实移动视口高度并限制横向溢出，以 100% 默认缩放打开即可完整阅读，无需手动缩小网页。
- 短屏手机可以自然纵向滚动当前内容；生成后的两页个人主页仍保留分页滑动。
- 已包含 1050px、700px 和 480px 三档响应式适配，并考虑全面屏安全区域。
